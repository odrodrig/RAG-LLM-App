...
3. Click the `Try it out` button and customize your request body:
    ...
    **NOTE**: This api will append documents to an existing index and pipeline with the same name if it finds them. 
    ...
    If you want to delete the documents in an index:
...
1. Authenticate the `queryWXDLLM` api by clicking the lock button to the right.  Enter the value you added for the `RAG_APP_API_KEY`.
